...
|---|---|---|---|
| **Environment Innovation** | 40% | Industrial methanol plant digital twin with Azure DT + multi-agent MARL + Monte Carlo sensor simulation + real-time 3D visualization | IN PROGRESS |
| **Storytelling** | 30% | HF blog + <2min video: AI agents controlling a chemical plant through a cloud digital twin in real time | NOT STARTED |
| **Reward Improvement** | 20% | GRPO training with Unsloth, real reward curves (before/after), 4 baseline agents vs trained | BASELINES DONE |
| **Training Pipeline** | 10% | Working Colab notebook with Unsloth 4-bit QLoRA GRPO against live physics environment | NOTEBOOK EXISTS |
...
| 1.7 | Populate training_plots/ with actual plots | ✅ | Agent | loss_curve.png, reward_curve.png, baseline_vs_trained.png |
...
| 1.12 | Install PyTorch CUDA on local machine | ❌ | Agent | Network dropped during 2.5GB download, retry needed |
...
|---|---|---|---|---|---|
| **2.1** | **DTDL Model Definition** | ⬜ | Agent | 1 hr | Create DTDL v3 JSON-LD models for all plant equipment |
...
| **2.3** | **Twin Graph Instantiation** | ⬜ | Agent | 1 hr | |
...
| **2.5** | **OpenEnv ↔ Azure DT Integration** | ⬜ | Agent | 2 hrs | |
...
| **2.7** | **Multi-Agent MARL on Azure DT** | ⬜ | Agent | 2 hrs | |
...
| # | Task | Status | Owner | Notes |
|---|---|---|---|---|
| 3.1 | Run train_grpo.ipynb on Colab A100/T4 OR HF GPU Space ($30 credits) | ⬜ | Bhavneet | CRITICAL — must produce real loss/reward plots |
| 3.2 | Replace placeholder training_plots/ with real outputs | ⬜ | Bhavneet | Blocked by 3.1 |
| 3.3 | Run before/after comparison: untrained vs trained agent | ⬜ | Bhavneet | Blocked by 3.1 |
...
| # | Task | Status | Owner | Time Est | Notes |
|---|---|---|---|---|---|
| **3.5.1** | **Create `reactor_sim_gpu.py`** | ⬜ | Agent | 2 hrs | PyTorch tensor version of reactor_sim.py |
...
| 3.5.3.2 | Benchmark: 1 env CPU vs 256 envs GPU | ⬜ | | | Target: 100x throughput improvement |
...
2. **🔴 CRITICAL**: Install PyTorch CUDA locally (downloading now — 40% done)
3. **🔴 CRITICAL**: Set up Azure DT instance → need Azure portal access NOW
4. **🟡 HIGH**: GPU-accelerate reactor_sim → `reactor_sim_gpu.py` (WRITTEN, needs PyTorch to test)
